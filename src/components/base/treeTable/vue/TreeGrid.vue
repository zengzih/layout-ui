<template>
  <el-table
    ...
    :stripe="true"
  >
    <el-table-column  @click="btn()" v-for="(column, index) in columns"  :align="getTheadAlign(column.align)" :fixed="column.fixed" :key="column.dataIndex"
      :label="column.text">
    ...
    <el-table-column label="操作" v-if="treeType === 'normal'" width="auto" align="center" :fixed="getToolTip()">
      <template scope="scope">
        <el-tooltip class="item" effect="light" v-for="btn in handleBtns" :content="btn.name" placement="left">
          <el-button
            @click.native="onBtnClickEvent(scope.$index, scope.row,btn.name)"
            type="text"
            size="small">
