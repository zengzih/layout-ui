    ...
    <div class="sc-theme is-standard">
		<sc-form-layout class="bmargin">
            <sc-form-layout-row>
				...
				<sc-form-layout-col class="right">
		...
		<div v-show="true" ref="queryShow"  id="queryShow" class="border ymargin padding bgcolor">
        ...
        <!-- 表格显示数据 -->
        <el-table :data="tableData" @sort-change="onSort" fit border stripe style="width:100%"
        :height="tableHeight">
            <el-table-column type="index" width="80"></el-table-column>
            <el-table-column prop="id" label="编号" width="150" sortable></el-table-column>
            <el-table-column prop="bankCode" label="银行代码" width="150"
            sortable></el-table-column>
            <el-table-column prop="cardNo" label="银行卡号" sortable
            ...
            <el-table-column prop="orgName" label="机构名称" width="150"></el-table-column>
            <el-table-column prop="createDateTime" label="创建日期"
            width="200"></el-table-column>
            <el-table-column prop="isDefaultName" label="是否默认" width="100"></el-table-column>
            <el-table-column fixed="right" label="操作">
                <template scope="scope">
                    <el-tooltip class="item" effect="light" v-if="actions.del.visible" :content="actions.del.text"
                    placement="left">
                        <el-button @click.native.prevent="onDeleteRow(scope.$index, tableData)"
                        type="text" size="small">
                            <i :class="actions.del.icon"></i>
                        </el-button>
                    </el-tooltip>
                    <el-tooltip class="item" effect="light" v-if="actions.edit.visible" :content="actions.edit.text"
                    placement="left">
                        <el-button @click.native.prevent="onEditRow(scope.$index, tableData)"
                        type="text" size="small">
                            <i :class="actions.edit.icon"></i>
                        </el-button>
                    </el-tooltip>
                    <el-tooltip class="item" effect="light" v-if="actions.read.visible" :content="actions.read.text"
                    placement="left">
                        <el-button @click.native.prevent="onReadRow(scope.$index, tableData)"
                        type="text" size="small">
        ...
        <!-- 弹出的表单 -->
        <el-dialog title="新增银行信息" v-model="dialogFormVisible" :close-on-click-modal="false">
            <el-form :inline="true" :model="formData" ref="formData">
				<sc-form-layout  label-width="small">
					<sc-form-layout-row>
						...
						<sc-form-layout-col>
							<el-form-item label="创建日期" prop="createDateTime" :label-width="formLabelWidth"
							:rules="[{ required: true, message: '创建日期不能为空'}]">
								<el-date-picker :disabled="state.createDateTime" @change="onDateChange($event,formData,'createDateTime')"
								v-model="formData.createDateTime" type="date" placeholder="选择日期" :picker-options="pickerOptions"></el-date-picker>
							</el-form-item>
						</sc-form-layout-col>
					...
					<sc-form-layout-row>
						...
						<sc-form-layout-col>
							...
							:rules="[{ required: true, message: '账户名称不能为空'}]">
								<el-input :disabled="state.accountName" v-model="formData.accountName"
            ...
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" v-if="actions.save.visible" @click="onSubmit('formData')"
                v-cloak>{{actions.save.text}}</el-button>
